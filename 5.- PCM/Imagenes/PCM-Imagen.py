...
class ImageApp:
    def __init__(self, master):
        ...
        self.combox['values'] = ("1", "2", "4", "8","16","32","64")  # Lista de opciones
    ...
    def quantize_image(self, event=None):
        ...
        if bits:
            if hasattr(self, 'gray_image'):  # Si la imagen está en escala de grises
                ...
                self.guardar_binario(quantized_image, txt_filename, bits)

                # Mostrar la imagen recuantizada
                self.photo = ImageTk.PhotoImage(quantized_image_pil)
                self.label.config(image=self.photo)
            else:
                messagebox.showinfo("Información", "La generación de PCM no está disponible en esta opción")
                if hasattr(self, 'black_white_image'):  # Si la imagen está en blanco y negro
    ...
    def quantize(self, image_data, bits):
        levels = 2 ** bits
        max_val = 255  # Valor máximo para una imagen en escala de grises
        quantized = np.floor((image_data.astype(float) / max_val) * levels) * (max_val / levels)
        return quantized.astype(np.uint8)

    def guardar_binario(self, quantized_image, filename, bits):
        with open(filename, 'w') as f:
            for pixel in quantized_image.flatten():
                # Convertir cada pixel a su representación binaria con el número de bits especificado
                binario = format(int(pixel * ((2 ** bits) - 1) / 255), f'0{bits}b')
                f.write(binario + '\n')
